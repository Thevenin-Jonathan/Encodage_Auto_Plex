...
def obtenir_fichiers(dossier):
    try:
        return {fichier for fichier in os.listdir(dossier) if os.path.splitext(fichier)[1].lower() in extensions}
...
def surveille_dossiers(dossiers_presets, file_encodage):
    ...
    fichiers_initiaux = {dossier: obtenir_fichiers(
        dossier) for dossier in dossiers_presets}
        ...
        for dossier, preset in dossiers_presets.items():
            ...
            if nouveaux_fichiers:
                for fichier in nouveaux_fichiers:
                    if fichier.endswith("_encoded.mp4"):
                        ...
                        continue
                    print(f"{horodatage()} 🆕 Nouveau fichier détecté dans {
                          dossier}: {fichier}")
            ...
            if fichiers_supprimes:
                for fichier in fichiers_supprimes:
                    print(f"{horodatage()} 🗑️ Fichier supprimé dans {
                          dossier}: {fichier}")
                    if dossier in fichiers_detectes and fichier in fichiers_detectes[dossier]:
                        fichiers_detectes[dossier].remove(fichier)
                    if dossier in fichiers_encodes and fichier in fichiers_encodes[dossier]:
